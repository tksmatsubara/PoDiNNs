...
class Dataset:
    """
    Mass-spring-damper system with moving boundary in absolute coordinate system
    """

    dataset_name = "msdBabs"
    ...
    params = argparse.Namespace(
        ...
        d=[
            lambda v: -0.10 * np.sign(v) * np.abs(v) ** (1 / 3),
    ...
    def __init__(self, base_dir="./", retry=False, **kwargs):
        ...
        v_b = self._external_flow(self.data_train.external_flow_param, self.data_train.t_eval, i=np.arange(self.data_train.u.shape[0]))
        q_b = self._external_position(self.data_train.external_flow_param, self.data_train.t_eval, i=np.arange(self.data_train.u.shape[0]))
        self.v_b_mean = v_b.reshape(-1).mean(keepdims=True)
        self.v_b_std = v_b.reshape(-1).std(keepdims=True)
        self.q_b_mean = q_b.reshape(-1).mean(keepdims=True)
        self.q_b_std = q_b.reshape(-1).std(keepdims=True)
        self.f_s_mean = np.zeros([0])
        self.f_s_std = np.zeros([1])

    def external_flow(self, t, i, train=True):
        return self.external_position(t=t, i=i, train=train, differential=True)

    def external_position(self, t, i, train=True, differential=False):
        data = self.data_train if train else self.data_test
        if not hasattr(data, "external_effort_param_gpu"):
            data.external_effort_param_gpu = tuple([torch.tensor(p, device=t.device, dtype=t.dtype) for p in data.external_flow_param])
        ground_freq, ground_amplitude, ground_phase = data.external_effort_param_gpu
        t = t.unsqueeze(-1)
        omega = 2 * torch.pi * ground_freq[i]
        if not differential:
            val = (ground_amplitude[i] * torch.sin(omega * t + ground_phase[i])).sum(-1).unsqueeze(-1)
        else:
            val = (ground_amplitude[i] * omega * torch.cos(omega * t + ground_phase[i])).sum(-1).unsqueeze(-1)
        return val
    ...
    def _external_flow(self, params, t, i=None):
        return self._external_position(params=params, t=t, i=i, differential=True)

    def _external_position(self, params, t, i=None, differential=False):
        ...
        if not differential:
            val = (ground_amplitude[i] * np.sin(omega * t + ground_phase[i])).sum(-1)
        else:
            val = (ground_amplitude[i] * omega * np.cos(omega * t + ground_phase[i])).sum(-1)
        if non_array_t:
            val = val[0]
        return val
    ...
    def visualize(self, base_dir, state_predicted):
        ...
        for idx_solution in range(self.data_test.u.shape[0]):
            ...
            q_b = self._external_position(self.data_test.external_flow_param, self.data_test.t_eval, i=idx_solution).flatten()
            ax.plot(t_eval, q_b, color="k", ls="-", label="q_b")
        ...
        try:
            ...
            for data in (data_test, data_train):
                ...
                for itr in range(data.u.shape[0]):
                    fig, ax = plt.subplots(1, 1, sharex=True, sharey=False, figsize=(6, 3), facecolor=None, dpi=300)
                    states = data.u[itr].T
                    q_b = self._external_position(data.external_flow_param, data.t_eval, i=itr).flatten()
                    t_eval = data.t_eval
                    ax.plot(t_eval, q_b, color="k", ls="-", label="q_b")
                    for i, s in enumerate(states):
                        if i < len(states) // 2:
    ...
    def _ode(self, t, state, external_flow_param):
        ...
        q_b = self._external_position(external_flow_param, t)  # volocities, moving ground
        v_b = self._external_flow(external_flow_param, t)  # volocities, moving ground
        q_dot = v_m
        v_d = [v_m[i] - v_m[i - 1] if i > 0 else v_m[i] - v_b for i in range(n)]
        q_k_rel = [q_k[i] - q_k[i - 1] if i > 0 else q_k[i] - q_b for i in range(n)]
